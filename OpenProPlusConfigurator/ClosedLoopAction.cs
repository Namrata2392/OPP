    ...
    public class ClosedLoopAction
    {
        ...
        public ClosedLoopAction()
        {
            ...
            try
            {
                ...
                //Nikita: 05/02/2020
                uccla.lvCLAItemSelectionChanged += new System.Windows.Forms.ListViewItemSelectionChangedEventHandler(this.lvCLA_ItemSelectionChanged);
                addListHeaders();
                fillOptions();
            }
            catch (Exception ex)
        ...
        private void lvCLA_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            try
            {
                if (uccla.lvCLA.SelectedItems.Count != 0)
                {
                    int ListIndex = uccla.lvCLA.FocusedItem.Index;
                    ListViewItem lvi = uccla.lvCLA.Items[ListIndex];

                    uccla.cb_AI1.SelectedItem = lvi.SubItems[1].Text;
                    uccla.cb_AI2.SelectedItem = lvi.SubItems[2].Text;
                    uccla.txtbx_High.Text = lvi.SubItems[5].Text;
                    uccla.textbx_High.Text = lvi.SubItems[5].Text;
                    uccla.textbx_Low.Text = lvi.SubItems[6].Text;
                    uccla.txtbx_Low.Text = lvi.SubItems[6].Text;
                                   
                    if ((Utils.IsGreaterThanZero(Convert.ToString(uccla.cb_AI1.SelectedItem))) && (Convert.ToString(uccla.cb_AI2.SelectedItem) == "0") || ((Utils.IsGreaterThanZero(Convert.ToString(uccla.cb_AI2.SelectedItem))) && (Convert.ToString(uccla.cb_AI1.SelectedItem)) == "0"))

                    {
                        uccla.pbOutput.Image = new Bitmap((Image)Properties.Resources.ResourceManager.GetObject("OR"));
                        uccla.pbOutput.SizeMode = PictureBoxSizeMode.StretchImage;
                    }
                    else
                    {
                        uccla.pbOutput.Image = new Bitmap((Image)Properties.Resources.ResourceManager.GetObject("AND"));
                        uccla.pbOutput.SizeMode = PictureBoxSizeMode.StretchImage;
                    }
                }
            ...
            { 
                ...
                if (mode == Mode.ADD)
                {
                    ...
                    Utils.CreateDI4CLA(Int32.Parse(claList[claList.Count - 1].CLAIndex));
                     //Nikita: 04/02/2020
                    //Nikita:05/02/2020
                    uccla.splitContainer1.Panel2Collapsed = false;
                    //Nikita: 10/02/20120
                    uccla.txtbx_High.Text = uccla.textbx_High.Text = claList[claList.Count - 1].High;
                    uccla.txtbx_Low.Text = uccla.textbx_Low.Text = claList[claList.Count - 1].Low;
                    if (Utils.AilistRegenerateIndex.Count > 0)
                    {         //nikita  : 11/02/2020                                 
                            foreach(AI ai in Utils.AilistRegenerateIndex.Where(x => Convert.ToInt32(x.AINo) > 0))
                            {
                                    uccla.cb_AI1.Items.Add(ai.AINo); 
                            }
                            //Utils.AilistRegenerateIndex.OfType<AI>().ToList().ForEach(x =>
                            //{
                            //    uccla.cb_AI1.Items.Add(x.AINo);
                            //});
                            //uccla.cb_AI1.DataSource = Utils.AilistRegenerateIndex;
                            uccla.cb_AI1.Items.Insert(0,"0");
                            uccla.cb_AI1.SelectedIndex = uccla.cb_AI1.FindString(claList[claList.Count - 1].AINo1);                         
                            uccla.cb_AI1.DisplayMember = "AINo";
                            uccla.cb_AI1.ValueMember = "AINo";
                       
                            foreach (AI ai in Utils.AilistRegenerateIndex.Where(x => Convert.ToInt32(x.AINo) > 0))
                            {
                                uccla.cb_AI2.Items.Add(ai.AINo);
                            }
                            //Utils.AilistRegenerateIndex.OfType<AI>().ToList().ForEach(x =>
                            //    {
                            //        uccla.cb_AI2.Items.Add(x.AINo);
                            //    });
                                //uccla.cb_AI2.DataSource = Utils.AilistRegenerateIndex;
                            uccla.cb_AI2.Items.Insert(0, "0");
                            uccla.cb_AI2.SelectedIndex = uccla.cb_AI2.FindString(claList[claList.Count - 1].AINo2);                        
                            uccla.cb_AI2.DisplayMember = "AINo";
                            uccla.cb_AI2.ValueMember = "AINo";                      
                    }
                    if ((Utils.IsGreaterThanZero(Convert.ToString(uccla.cb_AI1.SelectedItem)) && Convert.ToString(uccla.cb_AI2.SelectedItem) == "0") || (Utils.IsGreaterThanZero(Convert.ToString(uccla.cb_AI2.SelectedItem)) && Convert.ToString(uccla.cb_AI1.SelectedItem) == "0"))
                ...
                else if (mode == Mode.EDIT)
                {
                    ...
                    //Nikita: 04/02/2020
                    //Nikita:05/02/2020                  
                    uccla.txtbx_High.Text = uccla.textbx_High.Text = claList[editIndex].High;
                    uccla.txtbx_Low.Text = uccla.textbx_Low.Text = claList[editIndex].Low;
                    if (Utils.AilistRegenerateIndex.Count > 0)
                    { //nikita: 11/02/2020
                        foreach (AI ai in Utils.AilistRegenerateIndex.Where(x => Convert.ToInt32(x.AINo) > 0))
                        {
